...
const RoomDetailsPage = () => {
  ...
  const handleConfirmBooking = async () => {
    ...
    if (!checkInDate || !checkOutDate) {
      setErrorMessage('Пожалуйства введите даты.');
    ...
    <div className="room-details-booking">
      {showMessage && (
        <p className="booking-success-message">
          Успешно зарегестрировано! Код подтверждения: {confirmationCode}. 
        </p>
      )}
      {errorMessage && (
      ...
      {bookings && bookings.length > 0 && (
        <div>
          <h3>Существующие данные о бронировании</h3>
          <ul className="booking-list">
            {bookings.map((booking, index) => (
              <li key={booking.id} className="booking-item">
                <span className="booking-number">Бронирование {index + 1} </span>
                <span className="booking-text">Начало: {booking.checkInDate} </span>
                <span className="booking-text">Конец: {booking.checkOutDate}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="booking-info">
        <button className="book-now-button" onClick={() => setShowDatePicker(true)}>Забронировать</button>
        <button className="go-back-button" onClick={() => setShowDatePicker(false)}>Назад</button>
        {showDatePicker && (
          <div className="date-picker-container">
            ...
            />
              ...
              <button className="confirm-booking" onClick={handleConfirmBooking}>Подтвердить бронирование</button>
            </div>
        )}
        {totalPrice > 0 && (
          <div className="total-price">
            <p>Цена: ${totalPrice}</p>
            <button onClick={acceptBooking} className="accept-booking">Принять бронирование</button>
          </div>
        )}
      </div>
    </div>
